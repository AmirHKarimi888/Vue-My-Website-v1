...
const addContent = () => {
    ...
    .then(() => {
        Action.post(url + "/posts", post.value)
    })
}
            ...
            <ul style="list-style-type: none;">
                <li v-for="section in post.content" :key="section">
                    <h1 v-if="section.type === 'h1' ? true : false">
                        {{ section.content }}
                        <v-icon v-if="!section.editCon" icon="mdi-pen" size="20" color="grey-darken-1" @click="startEditContent(section.id)"></v-icon>
                        <v-icon v-if="section.editCon" icon="mdi-pen" size="20" color="success" @click="editContent(section.id)"></v-icon>
                        <v-icon icon="mdi-delete" size="20" color="grey-darken-1" @click="deleteContent(section.id)"></v-icon>
                    </h1>
